...
const workInstallFile = async (workData: interfaceBackgroundRenderWork) => {
  ...
  if(workData.data){
    ...
    const reportTimer = setInterval(() => {
      ...
      workData.progress = unpackManager.getInstallationProgress()
      ipcRenderer.send("asynchronous-message", workData)
    }, 1000)

    // Wait for the unpacker to finish
    await unpackManager.installSingle(workData.data.installationItem.folderPath,workData.data.installationItem.folderToCopyPath)
  ...
  // If the data isn't set for some reason, terminate and throw an orderly error.
    ...
    const newCommand: interfaceBackgroundRenderWork = {
      ...
      status: "finished",
      progress: 0,
      command: "ERROR IN: installFile-work"
    }
...
export default workInstallFile
