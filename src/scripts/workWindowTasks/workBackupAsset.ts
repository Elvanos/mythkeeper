...
const workBackupAsset = async (workData: interfaceBackgroundRenderWork) => {
  ...
  if(workData.data){
    ...
    const reportTimer = setInterval(() => {
      ...
      workData.progress = backupManager.getBackupProgress()
      ipcRenderer.send("asynchronous-message", workData)
    }, 1000)

    // Wait for the backup to finish
    await backupManager.backupSingle(workData.data.folderName, workData.data.mkPaths)
  ...
  // If the data isn't set for some reason, terminate and throw an orderly error.
    ...
    const newCommand: interfaceBackgroundRenderWork = {
      ...
      status: "finished",
      progress: 0,
      command: "ERROR IN: backupAsset-work"
    }
...
export default workBackupAsset
